  ...
  if (!currentQuestion) {
    return (
      <div className="flex flex-col items-center justify-center h-screen p-4 text-center">
        ...
        <p className="text-gray-500 mb-6">他のカテゴリを試してみてください。</p>
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-bold text-indigo-700">{category}</h1>
        <span className="text-lg font-mono bg-gray-200 text-gray-700 px-3 py-1 rounded-md">
          {currentQuestionIndex + 1} / {quizQuestions.length}
        </span>
      </div>

      <div className="bg-white p-6 rounded-lg shadow-md">
        <p className="text-lg leading-relaxed text-gray-800">{currentQuestion.question}</p>
      </div>
      ...
      {showExplanation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4 text-gray-800">解説</h2>
            <div className="space-y-4">
              <p className="font-semibold">正解: {currentQuestion.choices[currentQuestion.correct_answer_index]}</p>

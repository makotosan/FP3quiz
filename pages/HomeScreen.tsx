  ...
  return (
    <div className="p-4 space-y-6">
      <header>
        ...
        <p className="text-gray-500">FP3級合格に向けて頑張りましょう！</p>
      </header>
      
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-lg font-semibold text-gray-700 mb-2">学習進捗</h2>
        <div className="flex items-center">
          ...
          <div className="w-1/2 space-y-3 pl-4">
            <div className="text-center">
              <p className="text-sm text-gray-500">総解答数</p>
      ...
      <div className="flex flex-col space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <Link to="/quiz/苦手問題" className="bg-white p-3 rounded-lg shadow text-sm font-semibold text-gray-700 hover:bg-gray-50">苦手問題</Link>
          <Link to="/quiz/過去問" className="bg-white p-3 rounded-lg shadow text-sm font-semibold text-gray-700 hover:bg-gray-50">過去問チャレンジ</Link>
          <Link to="/quiz/お気に入り" className="bg-white p-3 rounded-lg shadow text-sm font-semibold text-gray-700 hover:bg-gray-50">お気に入り</Link>
        </div>
      </div>
      
      <div className="space-y-3">
        <h2 className="text-lg font-semibold text-gray-700">分野別進捗</h2>
        {categories.map((cat) => {
          const stats = category_stats[cat] || { answered: 0, correct: 0 };
          const progress = stats.answered > 0 ? (stats.correct / stats.answered) * 100 : 0;
          return (
            <Link to={`/quiz/${encodeURIComponent(cat)}`} key={cat} className="block bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow">
